...
describe('TrackingService', () => {
  ...
  describe('createSession', () => {
    it('should create a new session if none exists', async () => {
      ...
      mockPrisma.trackingSession.create.mockImplementation(({ data }) => Promise.resolve({ id: 's1', ...data }));
    ...
    it('should update last-touch if session exists and UTMs change', async () => {
      ...
      mockPrisma.trackingSession.update.mockImplementation(({ data }) => Promise.resolve({ ...existingSession, ...data }));
  ...
  describe('trackEvent', () => {
    it('should update lastSeenAt when tracking an event', async () => {
      mockPrisma.trackingSession.findUnique.mockResolvedValue({ id: 's1', lastSeenAt: new Date() });
      ...
      expect(mockPrisma.trackingSession.update).toHaveBeenCalledWith(expect.objectContaining({
        where: { id: 's1' },
