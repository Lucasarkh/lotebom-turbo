  ...
  <div class="pub-page">
    ...
    >
      <div class="pub-error-card card">
        ...
        >
          <i
            class="bi bi-arrow-left-short back-nav-icon"
            aria-hidden="true"
          ></i>
    ...
    <template v-else-if="lot">
      ...
      <Transition name="v4-sales-motion-fade">
        <div
          v-if="currentSalesNotice"
          class="v4-sales-motion-toast"
          aria-live="polite"
          aria-atomic="true"
        >
        ...
        <div class="main-content-flow-v4">
          <div class="page-container-v4 split-view">
            ...
            <main class="content-col">
              ...
              >
                ...
                <div class="gallery-v4">
                  <div
                    v-for="(m, i) in galleryMedias"
                    :key="i"
                    class="gallery-tile"
                    :class="{ main: i === 0 }"
                    @click="
                      () => {
                        openLightbox(Number(i));
                        tracking.trackClick('Galeria: Abrir Foto', 'GALLERY');
                      }
                    "
                  >
                    <img
              ...
              >
                ...
                <div
                  class="panorama-container-v4"
                  :class="{ 'is-interaction-disabled': !isPanoramaInteractive }"
                  @click="
                    tracking.trackClick('Panorama: Interação 360', 'VIEW_360')
                  "
                >
                  <PanoramaViewer :panorama="lotPanorama" />
                  <div
                    v-if="showPanoramaTouchCta"
                    class="interaction-gate-v4"
                  >
                    <button
                      type="button"
                      class="interaction-gate-v4__btn"
                      @click.stop.prevent="enablePanoramaInteraction"
                    >
                      Ver 360 interativo
                    </button>
                    <p class="interaction-gate-v4__hint">
                      Ative para girar a vista 360. Desative para continuar
                      rolando a pagina.
                    </p>
                  </div>
                </div>
              ...
              >
                ...
                <div class="specs-grid-v4">
                  ...
                  >
                    ...
                    <span class="s-value">
                      {{ formatCurrencyToBrasilia(details?.pricePerM2) }}
                    </span>
                  </div>
                  <div
                    class="spec-entry"
                    v-if="details?.frontage"
                  >
                  ...
                  >
                    ...
                    <span class="s-value">
                      {{ slopeLabel(details?.slope) }}
                    </span>
                  </div>
                  <div
                    class="spec-entry"
                    v-if="details?.hasViela"
                  >
                    <span class="s-label">Viela</span>
                    <span class="s-value">
                      Sim{{
                        details?.vielaWidth ? ` (${details.vielaWidth} m)` : ""
                      }}{{
                        details?.vielaSide === "LEFT"
                          ? " — Lado Esquerdo"
                          : details?.vielaSide === "RIGHT"
                            ? " — Lado Direito"
                            : details?.vielaSide === "BACK"
                              ? " — Fundos"
                              : ""
                      }}
                    </span>
                ...
                <div
                  v-if="hasUrbanisticRestrictions"
                  class="specs-subsection-v4"
                >
                  <div class="subsection-title">Restrições Urbanísticas</div>
                  <div class="specs-grid-v4">
                    <div
                      class="spec-entry"
                      v-if="details?.recuoFrontal"
                    >
                    ...
                    <div
                      class="spec-entry"
                      v-if="details?.coeficienteAproveitamento"
                    >
                      <span class="s-label">Coef. de Aproveitamento</span>
                      <span class="s-value">
                        {{ details.coeficienteAproveitamento }}
                      </span>
                    </div>
                    <div
                      class="spec-entry"
                      v-if="details?.gabaritoMaximo"
                    >
                      <span class="s-label">Gabarito Máximo</span>
                      <span class="s-value">
                        {{ details.gabaritoMaximo }} m
                      </span>
                    </div>
                    <div
                      class="spec-entry"
                      v-if="details?.taxaPermeabilidade"
                    >
                      <span class="s-label">Taxa de Permeabilidade</span>
                      <span class="s-value">
                        {{ details.taxaPermeabilidade }}%
                      </span>
                    </div>
                    <div
                      class="spec-entry"
                      v-if="details?.zoneamento"
                    >
                ...
                >
                  ...
                  <div class="specs-grid-v4">
                    ...
                    <div
                      class="spec-entry"
                      v-if="details?.inscricaoImobiliaria"
                    >
                      <span class="s-label">Inscrição Imobiliária</span>
                      <span class="s-value">
                        {{ details.inscricaoImobiliaria }}
                      </span>
                    </div>
                  </div>
                  <div
                    v-if="details?.confrontacoes"
                    class="notes-box-v4 mt-3"
                  >
              ...
              >
                ...
                <div class="lot-plant-map-frame">
                  ...
                  <div
                    v-if="showPlantMapTouchCta"
                    class="interaction-gate-v4"
                  >
                    <button
                      type="button"
                      class="interaction-gate-v4__btn"
                      @click.stop.prevent="enablePlantMapInteraction"
                    >
                      Ver mapa interativo
                    </button>
                    <p class="interaction-gate-v4__hint">
                      Ative para arrastar e dar zoom. Desative para voltar a
                      rolar a página.
                    </p>
                  </div>
                </div>
                  ...
                  <div class="sim-header">
                    <div class="h-item">
                      ...
                      <span
                        ...
                        "
                      >
                        {{ formatCurrencyToBrasilia(details?.price || 0) }}
                      </span>
                    </div>
                  </div>
                  ...
                  <div class="sim-body">
                    ...
                    <div class="input-group-v4">
                      ...
                      <div class="ig-flex">
                        ...
                        >
                          <input
                            v-model.number="simDownPaymentPercent"
                            type="number"
                            step="0.1"
                            @input="
                              () => {
                      ...
                      <small class="ig-hint">
                        Entrada mínima:
                        {{ formatCurrencyToBrasilia(minDownPaymentValue) }} ({{
                          project?.minDownPaymentPercent || 10
                        }}%)
                      </small>
                    </div>
                    ...
                    <!-- Installments Slider -->
                    <div
                      class="input-group-v4"
                      style="margin-top: 32px"
                    >
                      <div class="ig-label">
                        Número de Parcelas:
                        <strong>{{ simMonths }} meses</strong>
                      </div>
                    ...
                    <div class="sim-result-v4">
                      ...
                      <div class="r-value">
                        {{ formatCurrencyToBrasilia(simResult) }}
                      </div>
                      <div class="r-detail">
                        <span v-if="project?.monthlyInterestRate > 0">
                          Juros: {{ project.monthlyInterestRate }}% am ({{
                            annualInterestRateEffective.toFixed(2)
                          }}% aa efetivo) + {{ project.indexer || "IGP-M" }}
                        </span>
                        <span v-else>
                          Sem juros + {{ project.indexer || "IGP-M" }}
                        </span>
                      </div>

                      <div
                        class="sim-summary-v4"
                        v-if="simMonths > 1"
                      >
                        <div class="summary-item">
                          <span class="s-l">
                            Investimento Total (Simulado):
                          </span>
                          <span class="s-v">
                            {{ formatCurrencyToBrasilia(simTotalInvested) }}
                          </span>
                        </div>
                        <div
                          class="summary-item"
                          v-if="simTotalInterest > 0"
                        >
                          <span class="s-l">
                            Custo de Financiamento (Juros):
                          </span>
                          <span class="s-v">
                            {{ formatCurrencyToBrasilia(simTotalInterest) }}
                          </span>
                        </div>
                      </div>
                    </div>
              ...
              <!-- Finance -->
              <section
                v-if="details?.paymentConditions"
                id="financiamento"
                class="section-v4"
              >
                <div class="section-title-v4">
                ...
                <div class="finance-card-v4">
                  ...
                  <div class="finance-body">
                    <!-- Ato (Single) -->
                    <div
                      v-if="details?.paymentConditions?.ato"
                      class="plan-group"
                    >
                      ...
                      <div class="plans-grid">
                        <div class="plan-pill">
                          ...
                          <span class="p-val">
                            R$
                            {{
                              details?.paymentConditions?.ato?.toLocaleString(
                                "pt-BR",
                                { minimumFractionDigits: 2 },
                              )
                            }}
                    ...
                    <div
                      v-if="
                        details?.paymentConditions?.atos &&
                        !details?.paymentConditions?.ato
                      "
                      class="plan-group"
                    >
                      <div class="group-label">Ato</div>
                      <div class="plans-grid">
                        <template
                          v-if="Array.isArray(details?.paymentConditions?.atos)"
                        >
                          <div
                            v-for="(p, idx) in details?.paymentConditions?.atos"
                            :key="'ato' + idx"
                            class="plan-pill"
                          >
                            ...
                            <span class="p-val">
                              R$
                              {{
                                (p.amount || 0).toLocaleString("pt-BR", {
                                  minimumFractionDigits: 2,
                                })
                              }}
                            </span>
                        ...
                        <div
                          v-else-if="
                            details?.paymentConditions?.atos?.count > 0
                          "
                          class="plan-pill"
                        >
                          <span class="p-qty">
                            {{ details?.paymentConditions?.atos?.count }}x ato
                          </span>
                          <span class="p-val">
                            R$
                            {{
                              (
                                (details?.paymentConditions?.atos?.total || 0) /
                                (details?.paymentConditions?.atos?.count || 1)
                              ).toLocaleString("pt-BR", {
                                minimumFractionDigits: 2,
                              })
                            }}
                    ...
                    <div
                      v-if="
                        details?.paymentConditions?.entrada ||
                        details?.paymentConditions?.entradas
                      "
                      class="plan-group"
                    >
                      ...
                      <div class="plans-grid">
                        <template
                          v-if="
                            Array.isArray(details?.paymentConditions?.entradas)
                          "
                        >
                          <div
                            v-for="(p, idx) in details?.paymentConditions
                              ?.entradas"
                            :key="'ent' + idx"
                            class="plan-pill"
                          >
                        ...
                        <div
                          v-else-if="
                            details?.paymentConditions?.entrada?.count > 0
                          "
                          class="plan-pill"
                        >
                          <span class="p-qty">
                            {{ details?.paymentConditions?.entrada?.count }}x
                            entrada
                          </span>
                          <span class="p-val">
                            R$
                            {{
                              (
                                (details?.paymentConditions?.entrada?.total ||
                                  0) /
                                (details?.paymentConditions?.entrada?.count ||
                                  1)
                              ).toLocaleString("pt-BR", {
                                minimumFractionDigits: 2,
                              })
                    ...
                    <div
                      v-if="
                        (
                          details?.paymentConditions?.parcelas ||
                          details?.paymentConditions?.installments
                        )?.length
                      "
                      class="plan-group"
                    >
                      ...
                      <div class="plans-grid">
                        <div
                          v-for="(plan, idx) in details?.paymentConditions
                            ?.parcelas ||
                          details?.paymentConditions?.installments"
                          :key="'par' + idx"
                          class="plan-pill"
                        >
                          <span class="p-qty">
                            {{ plan.months || plan.count || plan.qty }}x mensais
                          </span>
                          <span class="p-val">
                            R$
                            {{
                              (plan.amount || plan.value || 0).toLocaleString(
                                "pt-BR",
                                { minimumFractionDigits: 2 },
                              )
                            }}
                    ...
                    <!-- Saldo do Saldo -->
                    <div
                      v-if="details?.paymentConditions?.saldo"
                      class="plan-group"
                    >
                      ...
                      <div class="plans-grid">
                        <div class="plan-pill">
                          ...
                          <span class="p-val">
                            R$
                            {{
                              details?.paymentConditions?.saldo?.toLocaleString(
                                "pt-BR",
                                { minimumFractionDigits: 2 },
                              )
                            }}
                    ...
                    <div
                      v-if="
                        details?.paymentConditions?.intermediarias ||
                        details?.paymentConditions?.balloons
                      "
                      class="plan-group"
                    >
                      ...
                      <div class="plans-grid">
                        ...
                        >
                          <div
                            v-for="(b, idx) in details?.paymentConditions
                              ?.intermediarias ||
                            details?.paymentConditions?.balloons"
                            :key="'bal' + idx"
                            class="plan-pill balloon"
                          >
                            <span class="p-qty">{{ b.count }}x reforços</span>
                            <span class="p-val">
                              R$
                              {{
                                (b.amount || 0).toLocaleString("pt-BR", {
                                  minimumFractionDigits: 2,
                                })
                              }}
                            </span>
                        ...
                        <div
                          v-else-if="
                            details?.paymentConditions?.intermediarias?.count >
                            0
                          "
                          class="plan-pill balloon"
                        >
                  ...
                  <div
                    v-if="
                      details?.paymentConditions?.observacoes?.length ||
                      (details?.paymentConditions?.notes &&
                        details?.paymentConditions?.notes !== '')
                    "
                    class="finance-note"
                  >
                    ...
                    <div class="note-content">
                      <template
                        v-if="
                          Array.isArray(details?.paymentConditions?.observacoes)
                        "
                      >
                        <p
                          v-for="(obs, i) in details?.paymentConditions
                            ?.observacoes"
                          :key="i"
                        >
                          {{ obs }}
                      ...
                      <p v-else>
                        {{
                          details?.paymentConditions?.notes ||
                          details?.paymentConditions?.observacoes
                        }}
                      </p>
                    </div>
                  </div>
            ...
            <aside class="sidebar-col">
              <div class="sticky-conversion-card">
                ...
                >
                  <div
                    v-if="showBookingSection"
                    class="booking-section-v4"
                  >
                    <div
                      v-if="isPreLaunchMode && priorityQueueSuccess"
                    ...
                    >
                      ...
                      <button
                        class="back-link"
                        @click="priorityQueueSuccess = false"
                      >
                        Fechar
                      </button>
                    </div>
                    ...
                    >
                      ...
                      <button
                        @click="
                          () => {
                            bookingMode = true;
                            priorityQueueSuccess = false;
                            tracking.trackClick(
                              bookingPrimaryClickLabel,
                              'CONVERSION',
                            );
                          }
                        "
                        class="cta-reserve-v4"
                      >
                        {{ bookingCtaLabel }}
                      </button>
                      <div
                        v-if="bookingFlowMode === 'RESERVATION'"
                        class="reserve-fee"
                      >
                        Taxa de reserva:
                        {{ formatCurrencyToBrasilia(reservationFeeValue) }}
                      </div>
                      ...
                      <div class="booking-header">
                        ...
                        >
                          <i
                            ...
                            aria-hidden="true"
                          ></i>
                          <span class="back-nav-label">Cancelar</span>
                        </button>
                  ...
                  >
                    ...
                    <template v-if="corretor">
                      ...
                      <a
                        :href="`https://wa.me/${corretor.phone?.replace(/\D/g, '') || project?.contactPhone?.replace(/\D/g, '')}`"
                        target="_blank"
                        class="wa-btn-v4"
                        @click="
                          tracking.trackWhatsappClick({
                            lotCode,
                            realtorName: corretor.name,
                          })
                        "
                      >
                  ...
                  >
                    ...
                    <p>
                      O corretor entrará em contato em breve via WhatsApp ou
                      e-mail.
                    </p>
                  </div>
                </div>
          ...
          <footer class="v4-footer">
            <div class="page-container-v4">
              <section
                v-if="otherLotsCarouselItems.length"
                class="other-assets-v4"
                aria-labelledby="other-lots-title"
              >
                ...
                <ClientOnly>
                  <div class="other-lots-carousel-bleed-v4">
                    ...
                    >
                      <SwiperSlide
                        v-for="otherLot in otherLotsCarouselItems"
                        ...
                        >
                          <div class="other-lot-card-head-v4">
                            <div class="other-lot-heading-v4">
                              <h4 class="other-lot-title-v4">
                                {{ resolveOtherLotDisplayName(otherLot) }}
                              </h4>
                            </div>
                            <span
                              class="other-lot-status-v4"
                              :class="otherLotStatusClass(otherLot)"
                            >
                              {{ otherLotStatusLabel(otherLot) }}
                            </span>
                          </div>

                          <p class="other-lot-meta-v4">
                            {{ formatOtherLotMeta(otherLot) }}
                          </p>

                          <div class="other-lot-stats-v4">
                            <div class="other-lot-stat-v4">
                              <span class="other-lot-stat-label-v4">
                                Área total
                              </span>
                              <strong>
                                {{ formatOtherLotArea(otherLot) }}
                              </strong>
                            </div>
                            <div class="other-lot-stat-v4">
                              <span class="other-lot-stat-label-v4">
              ...
              <div
                v-if="project"
                class="v4-footer-inner"
              >
                <div class="v4-footer-brand">
                  <span class="v4-footer-tenant">
                    {{ project.tenant?.name }}
                  </span>
                  <span class="v4-footer-project">
      ...
      <div
        v-if="idealLotAvailableTags.length > 0"
        class="v4-floating-cta"
      >
        <button
          class="v4-cta-btn-animated"
          @click="
            () => {
              tracking.trackClick('CTA Flutuante Lote: Encontrar Lote Ideal');
              toggleIdealLotModal();
            }
          "
        >
          <div class="v4-cta-inner">
            <span class="v4-cta-icon-spark">
              <i
                class="bi bi-search"
                aria-hidden="true"
              ></i>
      ...
      <Transition name="fade-lot-modal">
        <div
          v-if="isIdealLotModalOpen"
          class="v4-filter-modal-overlay"
        >
          <div class="v4-filter-modal-card">
            <div class="v4-modal-header">
              <h3 class="v4-modal-title">Lote Ideal</h3>
              <button
                class="v4-modal-close"
                @click="toggleIdealLotModal"
              >
                ✕
              </button>
            </div>
            <div class="v4-modal-body">
              <p style="margin-bottom: 24px; color: #86868b; font-size: 15px">
                Escolha as características para encontrar o seu lote ideal.
              </p>
              ...
              <div class="v4-modal-options">
                <label class="v4-modal-option">
                  <input
                    type="checkbox"
                    v-model="idealLotExactMatch"
                  />
                  <div class="v4-option-info">
                    <span class="v4-option-title">Correspondência Exata</span>
                    <span class="v4-option-desc">
                      Mostrar apenas lotes com todos os selos selecionados.
                    </span>
                  </div>
                </label>
              </div>
            ...
            <div class="v4-modal-footer">
              <button
                class="v4-btn-modal-search"
                @click="applyIdealLotFilters"
              >
                {{
                  idealLotSelectedTags.length
                    ? `Ver ${idealLotFilteredCount} unidades compatíveis`
                    : "Ver todas as unidades"
                }}
              </button>
              <button
                v-if="idealLotSelectedTags.length"
                class="v4-btn-modal-clear"
                @click="idealLotSelectedTags = []"
              >
                Limpar seleção
              </button>
            </div>
...
<script setup lang="ts">
  ...
  const projectSlug = computed(
    () =>
      (tenantStore.config?.project?.slug ||
        props.slug ||
        route.params.slug ||
        "") as string,
  );
  ...
  const previewId = computed(
    () => (props.id || route.query.previewId) as string,
  );
  ...
  const lotCode = computed(() => {
    ...
    if (route.params.code)
      return decodeURIComponent(route.params.code as string);
  ...
  const isSameLotIdentifier = (a?: string | null, b?: string | null) =>
    !!normalizeLotIdentifier(a) &&
    normalizeLotIdentifier(a) === normalizeLotIdentifier(b);

  const otherLotSlideKey = (lot: any) =>
    String(lot?.id || lot?.code || lot?.name || "");
  ...
  const formatOtherLotMeta = (lot: any) => {
    ...
    return parts.length
      ? parts.join(" · ")
      : "Consulte disponibilidade e condições comerciais";
  };
  ...
  const lotPlantMap = computed(() => {
    ...
    const relevantHotspots = plantMap.value.hotspots.filter(
      (h) => h.linkId === lotId || (h.label && h.label === lotLabel),
    );
  ...
  const projectUrl = computed(() => {
    ...
    return corretorCode
      ? `${base}${base.includes("?") ? "&" : "?"}c=${corretorCode}`
      : base;
  });
  ...
  const currentLotPath = computed(() => {
    ...
    const lotSegment = lotCode.value
      ? `/${encodeURIComponent(lotCode.value)}`
      : "";
    const path = `${basePath}${lotSegment}`;

    return corretorCode
      ? `${path}?c=${encodeURIComponent(corretorCode)}`
      : path;
  });
  ...
  async function handlePaymentReturn() {
    ...
    try {
      ...
      bookingError.value =
        "Pagamento recebido, mas a reserva ainda está em processamento.";
    } catch (e: any) {
      bookingError.value =
        e.message || "Não foi possível confirmar o pagamento agora.";
    } finally {
  ...
  const allProjectLots = computed(() => {
    ...
    const fromElements = ((project.value as any).mapElements || []).map(
      (e: any) => ({
        id: e.id,
    ...
    const fromMapData = mapData
      ? Array.isArray(mapData.lots)
        ? mapData.lots.map(([, l]: [any, any]) => ({
            id: l.id,
            code: l.code || l.label || l.id,
          }))
        : mapData.lots
          ? Object.values(mapData.lots).map((l: any) => ({
              id: l.id,
              code: l.code || l.label || l.id,
            }))
          : []
    ...
    const fromTeaser = ((project.value as any).teaserLots || []).map(
      (l: any) => ({
        id: l.id,
  ...
  const lot = computed(() => {
    ...
    if (mapData) {
      try {
        ...
        if (found) {
          const publicCandidate = (publicLotCandidates.value || []).find(
            (l: any) =>
          ...
          let finalAreaM2 =
            Number(found.area) > 0 ? Number(found.area) / (PPM * PPM) : 0;
          ...
          const finalFrontage =
            found.manualFrontage != null
              ? Number(found.manualFrontage)
              : Number(found.frontage) > 0
                ? Number(found.frontage) / PPM
                : 0;
          ...
          return {
            ...
            sideMetrics: Array.isArray(found.sideMetrics)
              ? found.sideMetrics
              : [],
            manualFrontage: found.manualFrontage ?? null,
            manualBack: found.manualBack ?? null,
            frontEdgeIndex:
              found.frontEdgeIndex ??
              found.metaJson?.frontEdgeIndex ??
              publicCandidate?.frontEdgeIndex ??
              null,
            frontAngleDeg:
              found.frontAngleDeg ??
              found.metaJson?.frontAngleDeg ??
              publicCandidate?.frontAngleDeg ??
              null,
            lotDetails: {
              status: (publicDetails?.status || found.status || "available")
                .toString()
                .toUpperCase(),
              price: publicDetails?.price ?? found.price ?? null,
              areaM2:
                publicDetails?.areaM2 ?? parseFloat(finalAreaM2.toFixed(2)),
              frontage:
                publicDetails?.frontage ?? parseFloat(finalFrontage.toFixed(2)),
              ...
              sideMetricsJson:
                publicDetails?.sideMetricsJson ?? found.sideMetrics ?? [],
              ...
              conditionsJson:
                publicDetails?.conditionsJson ?? found.conditionsJson ?? [],
              paymentConditions:
                publicDetails?.paymentConditions ??
                ((typeof found.paymentConditions === "string"
                  ? JSON.parse(found.paymentConditions)
                  : found.paymentConditions) ||
                  null),
              panoramaUrl:
                publicDetails?.panoramaUrl ?? found.panoramaUrl ?? null,
              medias: Array.isArray(publicDetails?.medias)
                ? publicDetails.medias
                : [],
              ...
              recuoFrontal:
                publicDetails?.recuoFrontal ?? found.recuoFrontal ?? null,
              recuoLateral:
                publicDetails?.recuoLateral ?? found.recuoLateral ?? null,
              recuoFundos:
                publicDetails?.recuoFundos ?? found.recuoFundos ?? null,
              taxaOcupacao:
                publicDetails?.taxaOcupacao ?? found.taxaOcupacao ?? null,
              coeficienteAproveitamento:
                publicDetails?.coeficienteAproveitamento ??
                found.coeficienteAproveitamento ??
                null,
              gabaritoMaximo:
                publicDetails?.gabaritoMaximo ?? found.gabaritoMaximo ?? null,
              taxaPermeabilidade:
                publicDetails?.taxaPermeabilidade ??
                found.taxaPermeabilidade ??
                null,
              zoneamento: publicDetails?.zoneamento ?? found.zoneamento ?? null,
              usoPermitido:
                publicDetails?.usoPermitido ?? found.usoPermitido ?? null,
              matricula: publicDetails?.matricula ?? found.matricula ?? null,
              inscricaoImobiliaria:
                publicDetails?.inscricaoImobiliaria ??
                found.inscricaoImobiliaria ??
                null,
              confrontacoes:
                publicDetails?.confrontacoes ?? found.confrontacoes ?? null,
            },
          };
        }
      } catch (e) {
        console.error("Error parsing mapData in lot page", e);
      }
    }

    // 3. Public lots endpoint fallback (new lightweight public payloads)
    const fromPublicLots = (publicLotCandidates.value || []).find(
      (l: any) =>
        isSameLotIdentifier(l.code, lotCode.value) ||
    ...
    if (fromPublicLots) {
      return {
        ...
        frontEdgeIndex:
          fromPublicLots.frontEdgeIndex ??
          fromPublicLots.mapElement?.metaJson?.frontEdgeIndex ??
          null,
        frontAngleDeg:
          fromPublicLots.frontAngleDeg ??
          fromPublicLots.mapElement?.metaJson?.frontAngleDeg ??
          null,
  ...
  const isPanoramaMedia = (media: any) => {
    ...
    if (
      url.includes("panorama_360") ||
      url.includes("panorama-360") ||
  ...
  const rawPanoramaImageUrl = computed(() => {
    ...
    const medias = Array.isArray(details.value?.medias)
      ? details.value.medias
      : [];
  ...
  const panoramaImageUrl = computed(() =>
    buildPanoramaProxyUrl(rawPanoramaImageUrl.value),
  );
  ...
  const hasUrbanisticRestrictions = computed(() =>
    Boolean(
      details.value?.recuoFrontal ||
      details.value?.recuoLateral ||
      details.value?.recuoFundos ||
  ...
  const hasSimulator = computed(() =>
    Boolean(project.value?.showPaymentConditions),
  );
  const hasFinanceTable = computed(() =>
  ...
  const sectionNavItems = computed(() => {
    const items = [
      ...
      },
      {
        ...
        shortLabel: "Galeria",
        mobileLabel: "Galeria",
        visible: galleryMedias.value.length > 0,
      },
      {
        id: "vista-360",
        href: "#vista-360",
        title: "Vista 360° do Lote",
        shortLabel: "360°",
        mobileLabel: "360°",
  ...
  const stickySectionNavItems = computed(() =>
  ...
  const preLaunchCaptureMode = computed(() =>
    project.value?.preLaunchCaptureMode === "RESERVATION"
      ? "RESERVATION"
      : "QUEUE",
  );
  ...
  const canJoinPreLaunchQueue = computed(() => {
    ...
    if (!isPreLaunchReservationMode.value)
      return details.value?.status === "AVAILABLE";
  ...
  const bookingFlowMode = computed(() =>
    canJoinPreLaunchQueue.value ? "QUEUE" : "RESERVATION",
  );
  ...
  const bookingPrimaryClickLabel = computed(() =>
    bookingFlowMode.value === "QUEUE"
      ? "Botão: Abrir Fila de Preferencia"
      : "Botão: Abrir Reserva Online",
  );
  ...
  const terrainLotData = computed(() => {
    ...
    return {
      ...current,
      frontEdgeIndex:
        current.frontEdgeIndex ?? current.metaJson?.frontEdgeIndex ?? null,
      frontAngleDeg:
        current.frontAngleDeg ?? current.metaJson?.frontAngleDeg ?? null,
    };
  });
  ...
  const showTerrainPreset3d = computed(() => {
    ...
    return Boolean(
      ...
      (Array.isArray((lot.value as any)?.polygon) &&
        (lot.value as any).polygon.length >= 3) ||
      (Array.isArray((lot.value as any)?.geometryJson?.points) &&
        (lot.value as any).geometryJson.points.length >= 6),
    );
  });
  ...
  const buildTrackedLotUrl = (
    source: "qr_code" | "share_button" | "whatsapp_share",
  ) => {
    ...
    try {
      ...
      const campaignBase = String(
        project.value?.slug || projectSlug.value || "projeto",
      )
        .trim()
        .replace(/\s+/g, "-")
        .toLowerCase();
      ...
      url.searchParams.set(
        "utm_medium",
        source === "qr_code" ? "offline" : "share",
      );
      url.searchParams.set("utm_campaign", `project_${campaignBase}`);
      url.searchParams.set(
        "utm_content",
        `lot_${lotLabel.replace(/\s+/g, "-").toLowerCase()}`,
      );
      return url.toString();
    } catch {
      return baseUrl;
  ...
  const trackedWhatsappUrl = computed(() =>
    buildTrackedLotUrl("whatsapp_share"),
  );

  const whatsappShareUrl = computed(() => {
    const message =
      `${lotShareMessage.value} ${trackedWhatsappUrl.value}`.trim();
  ...
  const salesMotionConfig = computed(() => {
    ...
    const rawDisplayMode = String(
      cfg.displayMode || cfg.frequency || "",
    ).toLowerCase();
    const showOnce =
      cfg.showOnce === true ||
    ...
    const defaultTemplates = [
      {
        id: "visitorsNow",
        text: "{{visitsNow}} pessoas estao vendo esse lote",
        enabled: true,
      },
    ];
      ...
      ? rawTemplates
          .map((tpl: any, idx: number) => {
            ...
            return {
              ...
              ranges:
                tpl.ranges && typeof tpl.ranges === "object"
                  ? tpl.ranges
                  : undefined,
            };
          })
  ...
  const salesMotionRangeFallback: Record<
    SalesMotionNumericToken,
  ...
  const salesMotionSessionStorageKey = computed(() => {
    const projectKey = String(
      project.value?.id || projectSlug.value || previewId.value || "unknown",
    );
    const lotKey = String(
      lot.value?.id || lot.value?.code || lotCode.value || "unknown",
    );
  ...
  const readSalesMotionSessionState = (): SalesMotionSessionState => {
    ...
    try {
      const raw = window.sessionStorage.getItem(
        salesMotionSessionStorageKey.value,
      );
      if (!raw) return {};
  ...
  const writeSalesMotionSessionState = (
    patch: Partial<SalesMotionSessionState>,
  ) => {
    if (!process.client) return;
    try {
  ...
  const resolveSalesMotionRange = (
    ...
    if (tpl?.manualRangeEnabled !== true) {
      return {
        ...
        max: Math.max(
          Math.max(0, Math.round(automatic.min)),
          Math.round(automatic.max),
        ),
      };
    }
  ...
  const salesMotionLotPool = computed(() => {
    const values = (allProjectLots.value || [])
  ...
  const buildSalesMotionNotice = (
    ...
    const baseViews = Math.max(
      ...
      Math.min(
        34,
        Math.round((allProjectAvailableLots.value.length || 8) * 0.9),
      ),
    );
    const baseVisits = Math.max(
      16,
      Math.min(260, Math.round((allProjectLots.value.length || 15) * 3.1)),
    );
    ...
    const fillTemplate = (tpl: any) => {
      ...
      const viewsRange = resolveSalesMotionRange(
        tpl,
        "viewsToday",
        automaticViewsRange,
      );
      const visitsRange = resolveSalesMotionRange(
        tpl,
        "visits24h",
        automaticVisitsRange,
      );
      const nowRange = resolveSalesMotionRange(
        tpl,
        "visitsNow",
        automaticNowRange,
      );
      const views = nextSmoothInt(
        baseViews,
        salesMotionLastViews,
        viewsRange.min,
        viewsRange.max,
      );
      const visits = nextSmoothInt(
        baseVisits,
        salesMotionLastVisits,
        visitsRange.min,
        visitsRange.max,
      );
      const nowUsers = nextSmoothInt(
        Math.max(2, Math.round(baseViews * 0.6)),
        salesMotionLastViews,
        nowRange.min,
        nowRange.max,
        0.13,
  ...
  const salesMotionTemplatesSignature = computed(() =>
    JSON.stringify(
      (salesMotionConfig.value.templates || []).map((tpl: any) => ({
        id: tpl.id,
        text: tpl.text,
        enabled: tpl.enabled,
  ...
  const startSalesMotion = () => {
    ...
    salesMotionLastViews.value = Number.isFinite(sessionViews)
      ? sessionViews
      : 0;
  ...
  const handleSalesMotionByProgress = () => {
    ...
    if (salesMotionShownCount.value >= salesMotionConfig.value.maxNotices)
      return;
    ...
    const nextMilestone = milestones.find(
      (m) => progress >= m && !salesMotionReachedMilestones.value.includes(m),
    );
  ...
  const otherLots = computed(() => {
    ...
    const fromTeaser = (((project.value as any).teaserLots || []) as any[]).map(
      (l: any) => ({
        id: l.id,
        code: l.code || l.name || l.id,
        ...
        lotDetails: {
          ...(l.lotDetails || {}),
          status: (
            (l.lotDetails?.status || "AVAILABLE") as string
          ).toUpperCase(),
        },
      }),
    );

    const fromPublicLots = ((publicLotCandidates.value || []) as any[]).map(
      (l: any) => ({
        id: l.id,
        code: l.code || l.name || l.id,
        name: l.name || l.code || "Lote",
    ...
    const currentLotCode =
      normalizeLotIdentifier(lot.value?.code) ||
    ...
    for (const item of merged) {
      ...
      const prevScore =
        Number(!!prev?.lotDetails?.price) + Number(!!prev?.lotDetails?.areaM2);
      const nextScore =
        Number(!!item?.lotDetails?.price) + Number(!!item?.lotDetails?.areaM2);
    ...
    return Array.from(deduped.values()).filter((e: any) => {
      const idMatch =
        normalizeLotIdentifier(e.id) &&
        normalizeLotIdentifier(e.id) === currentLotId;
      const codeMatch =
        normalizeLotIdentifier(e.code || e.name) &&
        normalizeLotIdentifier(e.code || e.name) === currentLotCode;
      if (idMatch || codeMatch) return false;

      return (
        (e.lotDetails?.status || "AVAILABLE").toUpperCase() === "AVAILABLE"
      );
    });
  });

  const otherLotsCarouselItems = computed(() =>
    otherLots.value.map((lot: any) => normalizeOtherLot(lot)).slice(0, 12),
  );

  const otherLotsCarouselShouldLoop = computed(
    () => otherLotsCarouselItems.value.length >= 5,
  );

  const allProjectAvailableLots = computed(() => {
    return allProjectLots.value.filter((item: any) => {
      return (
        String(item?.lotDetails?.status || "AVAILABLE").toUpperCase() ===
        "AVAILABLE"
      );
    });
  });
  ...
  const idealLotAvailableTags = computed(() => {
    ...
    for (const item of allProjectAvailableLots.value) {
      const tags = Array.isArray(item?.lotDetails?.tags)
        ? item.lotDetails.tags
        : [];
  ...
  const idealLotFilteredCount = computed(() => {
    if (idealLotSelectedTags.value.length === 0)
      return allProjectAvailableLots.value.length;
  ...
  const simResult = computed(() => {
    ...
    const downPayment = Math.max(
      simDownPayment.value,
      minDownPaymentValue.value,
    );
  ...
  const updateDownPaymentFromPercent = () => {
    ...
    simDownPayment.value =
      details.value.price * (simDownPaymentPercent.value / 100);
  };

  const updatePercentFromDownPayment = () => {
    if (!details.value?.price) return;
    simDownPaymentPercent.value = Number(
      ((simDownPayment.value / details.value.price) * 100).toFixed(1),
    );
  };
  ...
  watch(
    ...
    (val) => {
      if (val) {
        simDownPaymentPercent.value =
          project.value?.minDownPaymentPercent || 10;
        updateDownPaymentFromPercent();
      }
    },
    { immediate: true },
  ...
  watch(
    () => simMonths.value,
    (v) => {
      if (v) simMonths.value = Math.min(v, maxInstallments.value);
    },
  );
  ...
  watch(
    () => gateLeadForm.value.phone,
    (v) => {
      if (v) gateLeadForm.value.phone = maskPhone(v);
    },
  );
  watch(
    () => reservationForm.value.phone,
    (v) => {
      if (v) reservationForm.value.phone = maskPhone(v);
    },
  );
  watch(
    () => reservationForm.value.cpf,
    (v) => {
      if (v) reservationForm.value.cpf = maskCpf(v);
    },
  );
  ...
  const isPlantMapInteractive = computed(
    () => !isTouchMobile.value || plantMapInteractionEnabled.value,
  );
  const isPanoramaInteractive = computed(
    () => !isTouchMobile.value || panoramaInteractionEnabled.value,
  );
  const showPlantMapTouchCta = computed(
    () => isTouchMobile.value && !plantMapInteractionEnabled.value,
  );
  const showPanoramaTouchCta = computed(
    () => isTouchMobile.value && !panoramaInteractionEnabled.value,
  );
  const showPlantMapDisableButton = computed(
    () => isTouchMobile.value && plantMapInteractionEnabled.value,
  );
  const showPanoramaDisableButton = computed(
    () => isTouchMobile.value && panoramaInteractionEnabled.value,
  );
  ...
  onMounted(async () => {
    ...
    try {
      const baseUrl = isPreview.value
        ? `/p/preview/${previewId.value}`
        : `/p/${projectSlug.value}`;
      ...
      if (!isPreview.value && lotCode.value) {
        try {
          ...
          const [allLotsResponse, searchedLotResponse] =
            await Promise.allSettled([
              fetchPublic(`/p/${projectSlug.value}/lots?limit=100&page=1`),
              fetchPublic(
                `/p/${projectSlug.value}/lots?search=${searchParam}&limit=20&page=1`,
              ),
            ]);

          const allLots =
            allLotsResponse.status === "fulfilled" &&
            Array.isArray(allLotsResponse.value?.data)
              ? allLotsResponse.value.data
              : [];
          const searchedLots =
            searchedLotResponse.status === "fulfilled" &&
            Array.isArray(searchedLotResponse.value?.data)
      ...
      if (p.status === "fulfilled") {
        ...
        getPublicPlantMap(p.value.id, isPreview.value)
            ...
            if (pm && pm.hotspots) {
              pm.hotspots = pm.hotspots.map((h) => {
                if (h.linkType === "LOTE_PAGE" && h.linkId) {
                  const found = (allProjectLots.value as any[]).find(
                    (l) => l.id === h.linkId,
                  );
                  if (found) {
          ...
          .catch((e) =>
            console.error("Error fetching plant map on lot page", e),
          );
      } else {
    ...
    try {
      const body: any = {
        ...
        message:
          ...
          `Quero mais informações sobre o lote ${lotCode.value}`,
  ...
  async function submitReservation() {
    if (
      bookingFlowMode.value === "RESERVATION" &&
      !validateCpf(reservationForm.value.cpf)
    ) {
...
<style scoped>
  ...
  .specs-subsection-v4 .subsection-title {
    font-size: 14px;
    font-weight: 700;
    color: var(--v4-text);
    text-transform: uppercase;
  ...
  .sticky-conversion-card::-webkit-scrollbar {
    width: 4px;
  }
  .sticky-conversion-card::-webkit-scrollbar-thumb {
    background: #d2d2d7;
    border-radius: 10px;
  }
  ...
  .ig-field:focus-within {
    border-color: var(--v4-primary);
    background: white;
    box-shadow: 0 0 0 4px rgba(var(--v4-primary-rgb), 0.1);
  }
  .ig-curr {
    font-size: 14px;
    font-weight: 700;
  ...
  .ig-input::-webkit-outer-spin-button,
  ...
  .range-slider-v4::-webkit-slider-thumb:hover {
    transform: scale(1.1);
  }
  ...
  .v4-modal-option input[type="checkbox"] {
    width: 20px;
    height: 20px;
    cursor: pointer;
  }
  ...
  .fade-lot-modal-enter-active,
  .fade-lot-modal-leave-active {
    transition: opacity 0.3s;
  }
  .fade-lot-modal-enter-from,
  .fade-lot-modal-leave-to {
    opacity: 0;
  }
</style>
